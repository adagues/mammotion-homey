...
class MammotionMQTTDirect extends EventEmitter {
  ...
  connect() {
    ...
    // Make clientId unique to avoid conflict with mobile app
    const uniqueClientId = this.clientId + '_homey_' + Math.random().toString(36).substring(2, 8);
    console.log(`MQTT Direct: connecting to ${url} as ${uniqueClientId}`);
    ...
    this.client = mqtt.connect(url, {
      ...
      reconnectPeriod: 30000, // 30s between reconnects
