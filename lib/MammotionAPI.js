...
class MammotionAPI {
   ...
   */
  async _aliyunIoTRequest(domain, apiPath, params = {}, apiVer = '1.0', iotToken = null) {
    const body = {
      id: this._getNonce(),
      version: '1.0',
    ...
    if (resp.status === 429) {
      console.log(`Aliyun API ${apiPath}: rate limited (429), retrying in 3s...`);
      await new Promise(r => setTimeout(r, 3000));
      // Retry once
      const resp2 = await fetch(url, { method: 'POST', headers, body: bodyStr });
      const text2 = await resp2.text();
      if (resp2.status === 429) {
        throw new Error(`Aliyun API ${apiPath}: still rate limited after retry`);
      }
      try { return JSON.parse(text2); } catch { throw new Error(`Aliyun API ${apiPath} retry non-JSON: ${text2.substring(0, 200)}`); }
    }
