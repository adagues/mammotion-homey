...
class MammotionAPI {
  ...
  async _aliyunIoTRequest(domain, apiPath, params = {}, apiVer = '1.0', iotToken = null) {
    ...
    try {
      ...
      throw new Error(`Aliyun API ${apiPath} returned non-JSON: ${text.substring(0, 200)}`);
    }
  ...
  async getDeviceProperties(iotId) {
    const domain = this.apiGatewayEndpoint;
    const data = await this._aliyunIoTRequest(domain, '/cloud/thing/properties/get', {
      iotId,
      propertyKeys: [
        'battery_level', 'device_state', 'work_mode',
        'blade_height', 'error_code', 'charging_state', 'speed',
      ],
    }, '1.0', this.iotToken);
    return data.data || {};
  }
