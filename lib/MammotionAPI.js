...
class MammotionAPI {
  ...
  async _aliyunIoTRequest(domain, apiPath, params = {}, apiVer = '1.0', iotToken = null, _retryCount = 0) {
    ...
    // Handle rate limiting - max 2 retries
    if (resp.status === 429) {
      if (_retryCount >= 2) {
        const respHeaders = Object.fromEntries(resp.headers.entries());
        console.log(`429 response headers:`, JSON.stringify(respHeaders));
        throw new Error(`Aliyun API ${apiPath}: rate limited after ${_retryCount + 1} attempts`);
      }
      const delay = 5000 * (_retryCount + 1); // 5s, 10s
      console.log(`Aliyun API ${apiPath}: rate limited (429), retry ${_retryCount + 1}/2 in ${delay/1000}s...`);
  ...
  async login(email, password) {
    ...
    // Small delay to avoid rate limiting on first commands
  ...
  async _completeAuth() {
    ...
    // Step 6: AEP handle
    await this.aepHandle();
    // Step 7: Create session by auth code
    await this.sessionByAuthCode();
  }
  ...
  async _listDevicesMammotion(iotEndpoint) {
    const resp = await fetch(iotEndpoint + '/v1/user/device/page', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ iotId: '', pageNumber: 1, pageSize: 100 }),
    });
    const data = await resp.json();
    console.log('Mammotion device list response:', JSON.stringify(data).substring(0, 500));
  ...
  // Uses the JWT iot endpoint directly with Bearer token

  async sendProtobufCommand(iotId, commandBytes) {
    const contentBase64 = commandBytes.toString('base64');

    // Decode JWT to get the iot endpoint
    const iotEndpoint = this._getIotEndpoint();
    if (!iotEndpoint) {
      throw new Error('No IoT endpoint available - JWT missing iot field');
    }
    ...
    const data = await resp.json();
    console.log(`mqtt_invoke response (${resp.status}):`, JSON.stringify(data));

    if (data.code !== 0 && data.code !== 200) {
      throw new Error(`mqtt_invoke failed: code=${data.code}, msg=${data.msg || JSON.stringify(data)}`);
    }
  ...
  async startMowing(iotId) {
    ...
    return this.sendProtobufCommand(iotId, cmd);
  }
