...
class MammotionAPI {
  ...
  async getRegion(countryCode = 'EU') {
    try {
      ...
      if (data.data) {
        ...
        console.log('Region info:', JSON.stringify(data.data));
  ...
  async aepHandle() {
    ...
    console.log('AEP response:', JSON.stringify(data).substring(0, 300));
    return data;
  }
  ...
  async _listDevicesAliyun() {
    ...
    console.log('Aliyun devices:', JSON.stringify(data.data.data.map(d => ({iotId: d.iotId, deviceName: d.deviceName, productKey: d.productKey}))));
  ...
  // Send command - try Aliyun cloud first, fallback to mqtt_invoke

  async sendProtobufCommand(iotId, commandBytes) {
    const contentBase64 = commandBytes.toString('base64');
    const deviceData = this._lastDeviceData || {};
    console.log('Sending command, iotId:', iotId, 'deviceName:', deviceData.deviceName, 'productKey:', deviceData.productKey);

    // Try Aliyun cloud command first
    try {
      const domain = this.apiGatewayEndpoint;
      if (domain && this.iotToken) {
        const data = await this._aliyunIoTRequest(domain, '/thing/service/invoke', {
          args: { content: contentBase64 },
          identifier: 'device_protobuf_sync_service',
          iotId,
        }, '1.0.5', this.iotToken);
        console.log('Aliyun command response:', JSON.stringify(data).substring(0, 300));
        return data;
      }
    } catch (err) {
      console.log('Aliyun command failed:', err.message, '- trying mqtt_invoke...');
    }

    // Fallback: mqtt_invoke via Mammotion endpoint
    const iotEndpoint = this._getIotEndpoint();
    if (!iotEndpoint) throw new Error('No command endpoint available');

    const requestId = Array.from({ length: 21 }, () => Math.floor(Math.random() * 10)).join('');
    const resp = await fetch(`${iotEndpoint}/v1/mqtt/rpc/thing/service/invoke`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${this.accessToken}`,
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        ...
        identifier: 'device_protobuf_sync_service',
        iotId,
        productKey: deviceData.productKey || '',
      }),
    });
    const data = await resp.json();
    console.log('mqtt_invoke response:', JSON.stringify(data).substring(0, 300));
    if (data.code !== 0 && data.code !== 200) {
      throw new Error(`Command failed: code=${data.code}, msg=${data.msg}`);
    }
    return data;
  }
