...
class MammotionAPI {
  ...
  async _aliyunIoTRequest(domain, apiPath, params = {}, apiVer = '1.0', iotToken = null) {
    ...
    // Handle rate limiting - retry with fresh signature
    if (resp.status === 429) {
      console.log(`Aliyun API ${apiPath}: rate limited (429), retrying in 5s...`);
      await new Promise(r => setTimeout(r, 5000));
      // Recursive call to regenerate headers/signature
      return this._aliyunIoTRequest(domain, apiPath, params, apiVer, iotToken);
    }
