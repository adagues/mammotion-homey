...
class AliyunMQTTTransport extends EventEmitter {
  ...
  connect() {
    ...
    // Try TLS on port 8883
    const url = `mqtts://${host}:8883`;

    console.log(`AliyunMQTT: connecting to ${host}:8883 clientIdBase=${this.config.clientIdBase}`);
    console.log(`AliyunMQTT: clientId=${clientId.substring(0, 60)}...`);
    console.log(`AliyunMQTT: username=${this.config.deviceName}&${this.config.productKey}`);

    // Disable TLS cert verification for Aliyun IoT
